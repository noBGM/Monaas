...
void radio_ble_loadPacket(uint8_t* packet, uint16_t len) {
    radio_vars.state  = RADIOSTATE_LOADING_PACKET;
    //radio_vars.payload[0]=0;
    ///< note: 1st byte should be the payload size (for Nordic), and
    ///   the two last bytes are used by the MAC layer for CRC
    if ((len > 0) && (len <= MAX_PACKET_SIZE)) {
        //radio_vars.payload[1]=len&0xff;
        radio_vars.payload[1] = len;
        memcpy(&radio_vars.payload[2], packet, len);
    }
                           ...
                           bool* pCrc) 
{
    ...
    // copy payload
    memcpy(pBufRead, &radio_vars.payload[2], len);
