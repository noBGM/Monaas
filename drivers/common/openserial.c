...
owerror_t openserial_printLog(
    ...
    switch (log_level) {
        ...
        case L_CRITICAL:
            ...
            leds_debug_on();
            //openserial_inhibitStop();//new_added
            ...
            openserial_mysf_printf("L_CRITICAL");
            break;
        default:
...
owerror_t openserial_root_printf(char *buffer, ...) {
...
owerror_t openserial_leader_printf(char *buffer, ...) {
...
owerror_t openserial_member_printf(char *buffer, ...) {
    if(center_vars.role == CENTER_ROLE_MEMBER){
#if BOARD_OPENSERIAL_PRINTF
        uint8_t  i;
        char *ptr, *tmp;
        char c;
 ...
 * @brief 从串口输入缓冲区读取数据
 * 
 * @param bufferToWrite 目标缓冲区,用于存储读取的数据
 * @param maxNumBytes  最大可读取的字节数
 * @return uint8_t 实际读取的字节数
 */
uint8_t openserial_getInputBuffer(uint8_t *bufferToWrite, uint8_t maxNumBytes) {
    uint8_t numBytesWritten;      // 实际写入的字节数
    uint8_t inputBufFillLevel;    // 输入缓冲区填充水平
    INTERRUPT_DECLARATION();       // 中断相关声明
    ...
    if (maxNumBytes < inputBufFillLevel - 1) {
        ...
        //<<<<<<<<<<<<<<<<<<<<<<<
        DISABLE_INTERRUPTS();              // 关中断保护数据拷贝
        memcpy(bufferToWrite,              // 目标缓冲区
               &(openserial_vars.inputBuf[1]), // 源数据(跳过第一个字节)
               numBytesWritten);           // 拷贝字节数
        ENABLE_INTERRUPTS();               // 恢复中断
        //>>>>>>>>>>>>>>>>>>>>>>>
    }

    return numBytesWritten;  // 返回实际读取的字节数
}
...
owerror_t internal_openserial_print(
    ...
    if(severity == SERFRAME_MOTE2PC_CRITICAL){
...
void openserial_handleRxFrame() {
    ...
    switch (cmdByte) {
        ...
        #endif
        #if CENTER_SCHEDULE
        case SERFRAME_PC2MOTE_CENTER:
            root_handleCommand(&openserial_vars.inputBuf[1], openserial_vars.inputBufFillLevel - 1);
            break;
        case SERFRAME_LEADER2MEMBER_SCHEDULE_TEST:
            leader_handleCommand(&openserial_vars.inputBuf[1], openserial_vars.inputBufFillLevel - 1);
            break;
        case SERFRAME_PC2MOTE_TASK:
            //root分发任务命令：指令 ['T',05,predefined task index]
            //清除任务命令：指令 ['T',06]
            root_handleCommand(&openserial_vars.inputBuf[1], openserial_vars.inputBufFillLevel - 1);
            break;
        case SERFRAME_PC2MOTE_TASK_LEADER:
            //LEADER分发任务测试命令：指令 ['L',01]
            leader_handleCommand(&openserial_vars.inputBuf[1], openserial_vars.inputBufFillLevel - 1);
            break;
        case SERFRAME_PC2MOTE_SHOW_ATTR:
            //显示常用属性状态:指令 ['A']，任意角色都可以使用
            //目前有：消息队列、
            center_handleCommand(&openserial_vars.inputBuf[1], openserial_vars.inputBufFillLevel - 1);
            break;
        #endif
    }
}
...
void isr_openserial_tx(void) {
    if (openserial_vars.ctsStateChanged == TRUE) {
        ...
        if (openserial_vars.outputBufIdxW != openserial_vars.outputBufIdxR) {
            ...
            //openserial_leader_printf("isr_openserial_tx: leds_all_toggle");
            //leds_all_toggle();//bug：这里会一直高频率执行
            openserial_vars.fBusyFlushing = FALSE;
        }
    }
}
...
uint8_t isr_openserial_rx(void) {
    ...
    //开始读到第一个HDLC_FLAG的时候以下面的if条件都不满足，
    //下一步执行openserial_vars.hdlcLastRxByte = rxbyte。
