...
*/
//修改了一下，原来是1024和0x3FF，现在改成4096和0xFFF
#define SERIAL_OUTPUT_BUFFER_SIZE 4096
#define OUTPUT_BUFFER_MASK       0xFFF
...
#define SERFRAME_PC2MOTE_CENTER                  ((uint8_t)'N')
#define SERFRAME_LEADER2MEMBER_SCHEDULE_TEST     ((uint8_t)'C')
#define SERFRAME_PC2MOTE_TASK                    ((uint8_t)'T')
#define SERFRAME_PC2MOTE_TASK_LEADER             ((uint8_t)'L')
#define SERFRAME_PC2MOTE_SHOW_ATTR               ((uint8_t)'A')
...
typedef struct {
    ...
    uint8_t inputBufFillLevel; //就是长度。
