...
open_addr_t* neighbors_getKANeighbor(uint16_t kaPeriod) {
    ...
    asn_t asn;
    open_addr_t addr;
    neighborRow_t *parent;

    #ifdef CENTER_SCHEDULE
    ...
    case CENTER_ROLE_LEADER:
        parent = center_leader_getParent();
        break;
    case CENTER_ROLE_MEMBER:
        parent = center_member_getParent();
        break;
    default:
        break;
    }
    //openserial_leader_printf("parent neigbor got, id:%d",parent->addr_64b.addr_64b[7]);
    //openserial_member_printf("parent neigbor got, id:%d",parent->addr_64b.addr_64b[7]);
    timeSinceHeard = ieee154e_asnDiff(&parent->asn);
    if (timeSinceHeard > kaPeriod) {
        // this neighbor needs to be KA'ed to
        #if DEBUG_KA
        openserial_leader_printf("Need to KA, last:%d, ka:%d",timeSinceHeard,kaPeriod);
        openserial_member_printf("Need to KA, last:%d, ka:%d",timeSinceHeard,kaPeriod);
        openserial_member_printf("parent neigbor got, id:%d",parent->addr_64b.addr_64b[7]);
        #endif
        return &(parent->addr_64b);
    }
    #elif
    ...
    if (isNeighbor(address) == FALSE) {
        ...
        while (i < MAXNUMNEIGHBORS) {
            if (neighbors_vars.neighbors[i].used == FALSE) {
                ...
                openserial_leader_printf("add neighbor id:%d",address->addr_64b[7]);
                openserial_member_printf("add neighbor id:%d",address->addr_64b[7]);
                switch (center_vars.role)
                {
                case CENTER_ROLE_ROOT:
                    break;
                case CENTER_ROLE_LEADER:
                    if(address->addr_64b[7] == ROOT_ID)
                        center_leader_setParent(&neighbors_vars.neighbors[i]);
                    break;
                case CENTER_ROLE_MEMBER:
                    if(address->addr_64b[7] == MEMBER_EXPECTED_GROUP_ID)
                        center_member_setParent(&neighbors_vars.neighbors[i]);
                    break;
                default:
                    break;
                }
