...
owerror_t schedule_removeActiveSlot(slotOffset_t slotOffset, cellType_t type, bool isShared, open_addr_t *neighbor) {
    ...
    openserial_mysf_printf("schedule_removeActiveSlot");
    ...
    if (entry_found == FALSE) {
        ENABLE_INTERRUPTS();
        LOG_ERROR(COMPONENT_SCHEDULE, ERR_FREEING_ERROR, (errorparameter_t) 0, (errorparameter_t) 0);
        return E_FAIL;
    }
...
void schedule_removeAllCellsExceptBeacon(void){
    uint8_t i;
    for (i = 0; i < MAXACTIVESLOTS; i++) {
        if(schedule_vars.scheduleBuf[i].type != CELLTYPE_TEMP)
...
void schedule_removeAllMoveableCells(void){
    uint8_t i,j;
    for (i = 0; i < MAXACTIVESLOTS; i++) {
        if(schedule_vars.scheduleBuf[i].type == CELLTYPE_OFF){
            continue;
        }
        //后续优化可以在scheduleEntry_t加一个字段skip，表示允许跳过remove。
        switch (center_vars.role)
        {
        case CENTER_ROLE_LEADER:
            if( (schedule_vars.scheduleBuf[i].slotOffset == LEADER_BROADCAST_SLOT && schedule_vars.scheduleBuf[i].channelOffset == LEADER_BORADCAST_CHANNEL) ||
                (schedule_vars.scheduleBuf[i].slotOffset == LEADER_DOWNLINK_SLOT && schedule_vars.scheduleBuf[i].channelOffset == LEADER_DOWNLINK_CHANNEL) ||
                (schedule_vars.scheduleBuf[i].slotOffset == LEADER_UPLINK_SLOT && schedule_vars.scheduleBuf[i].channelOffset == LEADER_UPLINK_CHANNEL) ||
                (schedule_vars.scheduleBuf[i].slotOffset == center_vars.groupId && schedule_vars.scheduleBuf[i].channelOffset == center_vars.groupId)) // EB cell
                    continue;
            else{
                for(j = 0; j < CENTER_MAX_MEMBERS; j++){
                    if((schedule_vars.scheduleBuf[i].slotOffset == LEADER_MEMBER_UPLINK_SLOT_BASE + j && schedule_vars.scheduleBuf[i].channelOffset == LEADER_MEMBER_UPLINK_CHANNEL_BASE) ||
                        (schedule_vars.scheduleBuf[i].slotOffset == LEADER_MEMBER_DOWNLINK_SLOT_BASE + j && schedule_vars.scheduleBuf[i].channelOffset == LEADER_MEMBER_DOWNLINK_CHANNEL_BASE))
                        break;
                }
                if (j < CENTER_MAX_MEMBERS)
                {
                    continue;
                }    
                if(schedule_vars.scheduleBuf[i].type == CELLTYPE_TEMP &&
        ...
        case CENTER_ROLE_MEMBER:
            if((schedule_vars.scheduleBuf[i].slotOffset == MEMBER_BROADCAST_SLOT && schedule_vars.scheduleBuf[i].channelOffset == MEMBER_BROADCAST_CHANNEL) ||
                (schedule_vars.scheduleBuf[i].slotOffset == MEMBER_LEADER_UPLINK_SLOT && schedule_vars.scheduleBuf[i].channelOffset == MEMBER_LEADER_UPLINK_CHANNEL) ||
                (schedule_vars.scheduleBuf[i].slotOffset == MEMBER_LEADER_DOWNLINK_SLOT && schedule_vars.scheduleBuf[i].channelOffset == MEMBER_LEADER_DOWNLINK_CHANNEL))
                continue;
            else{
                if(schedule_vars.scheduleBuf[i].type == CELLTYPE_TEMP &&
                    ieee154e_isSynch()){
...
bool schedule_hasNegotiatedCellToNeighbor(open_addr_t *neighbor, cellType_t cell_type) {
    ...
    for (i = 0; i < MAXACTIVESLOTS; i++) {
        // if(schedule_vars.scheduleBuf[i].channelOffset == 6 && schedule_vars.scheduleBuf[i].slotOffset == 6){
        //     openserial_member_printf("%d,%d,%d,%d",
        //         schedule_vars.scheduleBuf[i].shared == FALSE,
        //         schedule_vars.scheduleBuf[i].type == cell_type,
        //         schedule_vars.scheduleBuf[i].neighbor.type == ADDR_64B,
        //         packetfunctions_sameAddress(neighbor, &schedule_vars.scheduleBuf[i].neighbor));
        //     openserial_member_printf("%d",schedule_vars.scheduleBuf[i].neighbor.addr_64b[7]);
        //     openserial_member_printf("%d",neighbor->addr_64b[7]);
        // }
