...
owerror_t sixtop_send(OpenQueueEntry_t *msg) {
    //openserial_leader_printf("sixtop_send");
    // set metadata
...
void task_sixtopNotifSendDone(void) {
    ...
    switch (msg->creator) {
        case COMPONENT_SIXTOP:
            ...
            //openserial_leader_printf("set busySending%s FALSE");
            // discard packets
        ...
        case IEEE154_TYPE_CMD:
            if (msg->length > 0) {
                if (msg->l2_frameType == IEEE154_TYPE_BEACON) {
                    ...
                    // free up the RAM
                    openserial_mysf_printf("free EB pkt just received with length > 0");
                    openqueue_freePacketBuffer(msg);
                    break;
                }
                // send to upper layer
...
#else
                iphc_receive(msg);
                center_receiveFrame(msg);
#endif
            } else {
                // free up the RAM
                openserial_mysf_printf("free pkt just received with length == 0");
                openqueue_freePacketBuffer(msg);
            }
            break;
...
owerror_t sixtop_send_internal(
        ...
        //openserial_leader_printf("sixtop_send_internal, IE %s",payloadIEPresent?"Exist":"Not Exist");
...
void timer_sixtop_sendEb_fired(void) {
    ...
    //     if(leader_vars.ebEnabled == TRUE){ //控制直接放在IEEE802154E中了
    //         sixtop_sendEB();
    //     }
    //     return;
    // }
...
port_INLINE void sixtop_sendEB(void) {
    ...
    //openserial_leader_printf("sixtop_sendEB");
    //openserial_root_printf("sixtop_sendEB");
    ...
    if (
        ...
        //openserial_leader_printf("stop in first IF %d%d%d%d",ieee154e_isSynch(),IEEE802154_security_isConfigured(),icmpv6rpl_getMyDAGrank(),icmpv6rpl_daoSent());
        // stop here
        return;
    }
    ...
    if (sixtop_vars.busySendingEB == TRUE) {
        ...
        //openserial_leader_printf("don't continue if I'm still sending a previous EB");
        return;
    }
    ...
    //openserial_leader_printf("got eb pkt");
    ...
    //不是在这里改下面的switch，而是修改ebIEsBytestream
...
port_INLINE void sixtop_sendKA(void) {
    ...
    //openserial_leader_printf("sixtop_sendKA");
    ...
    if (sixtop_vars.busySendingKA == TRUE) {
        ...
        //openserial_leader_printf("I'm still sending a KA");
        //openserial_member_printf("I'm still sending a KA");
        return;
    }
    ...
    if (kaNeighAddr == NULL) {
        ...
        //openserial_leader_printf("I don't need to send KA to my neighbor");
        //openserial_member_printf("I don't need to send KA to my neighbor");
        return;
    }
    ...
    if (schedule_hasNegotiatedCellToNeighbor(kaNeighAddr, CELLTYPE_TX) == FALSE) {
        ...
        #if DEBUG_KA
        openserial_leader_printf("I have no schedule slots to the neighbor, id:%d",kaNeighAddr->addr_64b[7]);
        openserial_member_printf("I have no schedule slots to the neighbor, id:%d",kaNeighAddr->addr_64b[7]);
        #endif
        return;
    }
