...
int mote_main(void) {
    uint8_t memberID = 1;
    uint8_t expectedGroup = CENTER_GROUP_HEAD;
    board_init();
    openstack_init(CENTER_ROLE_MEMBER,memberID,expectedGroup);
    //while(1){
    openserial_member_printf("member_main");
    //  some_delay();
    //}
    scheduler_start();
